        ...
        stage('Parallel Unix and Windows Build') {
            steps {
                parallel unix: {
                    node("${env.NIX_LABEL}") {
                        checkout scm
                        withEnv(["JAVA_HOME=${tool('JDK 1.8 (latest)')}", "PATH+MAVEN=${tool('Maven 3.5.0')}/bin:${env.JAVA_HOME}/bin"]) {
                            sh "mvn clean install jacoco:report -B -U -e -fae -V -P run-its -Dintegration-test-port=8084   \\\"-Djdk.home=${tool('JDK 1.9 (latest)')}\\\""
                        }
                ...
                windows: {
                    node("${env.WIN_LABEL}") {
                        checkout scm
                        withEnv(["JAVA_HOME=${tool('JDK 1.8 (latest)')}", "PATH+MAVEN=${tool('Maven 3.5.0 (Windows)')}\\bin;${env.JAVA_HOME}\\bin"]) {
                            bat "mvn clean install jacoco:report -B -U -e -fae -V -P run-its -Dintegration-test-port=8084"
                        }
                    }
                }
            }
